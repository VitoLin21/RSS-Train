...
from utilities.helpers import softmax_helper_dim0
...
class LabelManager(object):
    ...
    def convert_probabilities_to_segmentation(self, predicted_probabilities: Union[np.ndarray, torch.Tensor]) -> \
        ...
        if self.has_regions:
            ...
            segmentation = (predicted_probabilities[0] > 0.5).astype(np.int8)
        return segmentation
    ...
    def convert_logits_to_segmentation(self, predicted_logits: Union[np.ndarray, torch.Tensor]) -> \
        ...
        # probabilities = (probabilities > 0.5).astype(np.int8)
        # return probabilities
